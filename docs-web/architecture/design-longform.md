...
> **读者导读（请先看这里）**  
> 人读入门请先走：[诉求与 3W](/why/three-w) → [两层模型](/architecture/model) → [执行管线](/architecture/pipeline)（本地 `npm run docs:dev`）。  
> **本文是附录**：统一公式推导、模式映射与接口草图，不是 Getting Started。对外叙事以文档站 Why / 原理章为准。
