# docs-cross-project-composability

## Purpose

VitePress user-journey narrative for cross-project Unit reuse, control-plane docs, Mastra comparison, and tier-1 IA (not full-site rewrite).
...
Documentation work for cross-project narrative SHALL rewrite key journey pages listed above; it MUST NOT require rewriting every architecture module 3W page or every HTTP route handbook page in full (link tweaks allowed).

#### Scenario: Module handbook not mandated rewrite
- **WHEN** a maintainer reviews docs scope for this capability
- **THEN** full rewrites of all module 3W / HTTP route pages are not required as part of this capability
