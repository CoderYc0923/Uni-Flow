...
文档与安装指南已把「在自有 TS 项目里 `npm install virtual-uni-flow`」写成目标路径，但包尚未正式发布到 npm，用户只能依赖 Git / `file:`。现在 Engine、YAML、CLI 与文档口径已就绪，需要把 `uni-flow` 作为可安装的 npm 包发出去。
...
- **执行发布：** 在维护者已登录 npm 的前提下发布 `uni-flow@0.1.0`（或设计约定的首发版本）；记录发布后验证步骤（`npm view` / 干净目录 `npm install virtual-uni-flow`）
...
- `docs-ts-complete-path`: 安装指南以 `npm install virtual-uni-flow` 为默认成功路径（不再把「未发布」写成常态）
