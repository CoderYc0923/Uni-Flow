...
- [ ] 4.1 更新 `.vitepress/config.ts` nav/sidebar：指南顺序按「安装 → 快速开始 → YAML → 跨项目 → uses → 跨语言(次)」
- [ ] 4.2 重写首页 hero/features/「从这里开始」：跨项目复用为主；去掉「跨语言」作为主 feature 标题
- [ ] 4.3 重写 `why/three-w.md`：Who/Why 纳入跨项目 Unit 复用
- [ ] 4.4 扩展 `why/vs-frameworks.md`：增加 Mastra 对照表与互补结论（不替代）
- [ ] 4.5 新增 `guide/cross-project.md`：四通道、`params`/`$profile`、Workflow-as-Unit vs workflow-run、链 demo
- [ ] 4.6 更新 `guide/uses.md` 决策树；`guide/cross-lang.md` 导语改为手段说明并链到跨项目页
- [ ] 4.7 更新示例索引与相关示例导语（单项目 vs 跨项目）；README / AGENTS 门户链接对齐
- [ ] 4.8 `npm run docs:build` 通过；抽查首页 / 3W / vs / 跨项目 / uses
