...
6. 更新文档口径（uses 决策树、cross-lang → cross-project）
...
| `docs-web/` | 定位、uses 决策树、cross-project 指南、profile 约定 |
| Demo | Workflow-as-Unit 最小示例（可基于 cross-lang greeter 扩展） |

## 9. Risks / Open points（已决策）
...
2. 文档明确「跨项目复用」主叙事；cross-lang 降为手段说明
3. 有可跑的 Workflow-as-Unit 示例
4. 现有 `npm test` / validate 不回归
